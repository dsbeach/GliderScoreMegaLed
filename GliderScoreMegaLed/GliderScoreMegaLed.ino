/*
    GliderScoreMegaLed.ino - DBeach 1/6/2019

    See https://github.com/dsbeach/GliderScoreMegaLed - hopefully the wiki will be updated by the time somebody reads this!
...
char displayBuf[CMD_BUF_SIZE];

void setup(void) {
  memset(cmdBuf, 0, CMD_BUF_SIZE);
  memset(displayBuf, 0, CMD_BUF_SIZE);
  u8g2.begin();
  Serial.begin(115200);
}
...
void loop(void) {
  ...
  do {
    if (readCmd(cmdBuf)) {
        ...
        Serial.print(cmdBuf);
        Serial.println("<");
        memcpy(displayBuf, cmdBuf, CMD_BUF_SIZE);
    }
    u8g2.setFont(u8g2_font_ncenB14_tr);
    u8g2.drawStr(0,15,displayBuf);
  } while ( u8g2.nextPage() );
  // delay(1000);
}

bool readCmd(char *cmdBuf) {
    static int pos = 0;
    char readch;
